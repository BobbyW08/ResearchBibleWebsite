...
**Known gaps, intentionally left open:** the pinned CTA panel's disclaimer copy/placement; whether Join the Group's `8`-parent framing is ever enforced as a real cap; panel 02's and panel 04's `BALL EXIT` values are this build's best-guess update for the reordered panels, not confirmed by Bobby; no real PDF yet.
