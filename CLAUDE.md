...
| `/about-bobby` | About Bobby page | Live, CMS-managed via the `about` singleton. The H1 ("About Bobby") and section structure are fixed, not CMS fields — see Homepage/About redesign notes below. Hero is two-column (`lg:grid-cols-2`) — heading/subhead/credential badge/CTA left-aligned on the left, a large (`h-64`–`h-80` responsive) circular photo offset to the right; `photo` is a real Keystatic image field, falling back to a placeholder circle until Bobby uploads one. `/about` 301s here |
...
- **`/services` (For Parents):** Hero — a large eyebrow/headline/subtitle intro ("For Parents" / "By the time most families find their way to me, they're already lost in the woods." / supporting paragraph, all sized up from the original draft copy) leads into the big visual headline ("We Build [photo] Your Path", `components/marketing/services/wedged-hero-headline.tsx`), followed by a closing paragraph + CTA continuing the same material. That headline component renders two breakpoint-specific visual treatments (both `aria-hidden`; a shared `sr-only` `<h1>We Build Your Path</h1>` is the actual page heading regardless of which is showing) plus **Arvo Bold** (`font-subtitle font-bold`) type throughout:
  - **`sm` and up — `DesktopWedgedHeadline`:** unpinned, tied to the first 500px of ordinary page scroll (`useScroll()`/`useTransform`). Both text halves and the photo are driven off one shared progress value: each half starts flush against its own screen edge via the "break a centered flex child out to the viewport edge" CSS trick (`translateX(calc(±50vw ∓ 50%))` — exact for any word length/viewport width, no measurement needed) and slides inward as the photo scales up, landing as one tight single-row line at up to roughly a third of viewport width.
  - **Below `sm` — `MobilePinnedWedgedHeadline`:** a true scroll-jack, matching bymonolog.com's own mobile behavior rather than shrinking the desktop layout down (a shrunk single-row layout couldn't fit both text halves + a growing photo on a phone width without reflow jank). A tall spacer (`h-[220vh]`) with a `sticky top-0 h-dvh` inner pins the section to the screen and "eats" scroll input; progress comes from `useScroll({ target: containerRef, offset: ["start start", "end end"] })`, mapping 0→1 across exactly the pinned scroll distance. "We Build" and "Your Path" render as two centered lines (not overlapping each other) that start pushed toward the pinned viewport's top/bottom edge and slide to center together as the photo grows between them, then release to normal scrolling once converged.
  
  The `[photo]` slot in both is `components/marketing/services/rotating-path-photo.tsx`, which crossfades through a fixed set of nature/trail photos (`assets/paths/*.jpg`, static imports) on a 3-second timer independent of scroll or viewport position — not a photo of Bobby. → "Start Walking Your Path" (the Stabilize/Connect/Structure/Adapt framework — this content lives here only, not duplicated on `/about-bobby`; this section alone uses an **inverted palette** — `bg-brand-charcoal` dark-gray field, `text-brand-red-bright` headers, off-white body copy at reduced opacity — see Brand System) → "What I Offer" (1:1 Sessions and Weekly Group are `available`; Cohorts is `comingSoon` with a "Join the waitlist" CTA that opens the Newsletter dialog; Live Q&As is `comingSoon` with its own interactive "Show Interest" email-capture widget — `components/marketing/services/interest-signup-widget.tsx`, `OfferCardData.cta.kind === "interest-signup"` — instead of the newsletter dialog: click slides open an email input, a valid email reveals a Submit button, submitting POSTs to `/api/interest-signups` (inserts into the `interest_signups` table, `source: "live_qa"`) and folds back into the button with a persistent green checkmark, tracked client-side via `localStorage` since there's no account system to look it up server-side) → closing CTA. A second "Talk to Bobby" CTA sits inside the Weekly Group card, in addition to the page's closing CTA — intentional, not a duplicate. The 1:1 Sessions and Weekly Group `OfferCard`s carry stable anchor IDs (`#one-on-one`, `#weekly-group` — `OfferCardData.anchorId`, rendered onto the `Card` with `scroll-mt-24`) so the homepage Start Here Row 2 panels ("Parents." / "Groups.") can deep-link straight to them.
...
- **About-Bobby photo** — resolved. `content/about/data.yaml`'s `photo` field is now set (`bobby-washburn.jpg`, in `public/images/about/`) and the page renders it — done by directly seeding the content file rather than through `/keystatic` (still blocked on the GitHub App env vars), which is fine since it's a one-time content write, not an ongoing CMS workflow.
- **Real BWPS logo SVGs** — resolved, live in `assets/logo/` and wired into `Logo` (see Homepage Structure #2's Logo Behavior). Several duplicate/alternate exports (`BWPS Logo G_OW_R.svg`, `BWPS Logo Main Gradient.svg`, etc.) also sit in `assets/logo/` unused — confirm with Bobby whether those should be removed or are a different lockup still to be wired in later.
- **Start Here Row 2 photos are temporary stock, not real:** `start-here.tsx`'s Parents/Groups/Organizations panels now show warm-toned Unsplash stock photos (hotlinked, see Homepage Structure #3) rather than the `PlaceholderPhoto` icon box, but they're still not Bobby's own photos of real parents/groups/office per `homepage-redesign-v3.md`'s "Still needed" list — swap the three URL constants in `ROW_TWO_PANELS` once real photos exist. (Distinct from the Bobby headshots wired into Quick Credential/About — these Row 2 photos are meant to depict parents/scenarios, not Bobby.)
