...
2. **Hero — "Proof Wall"** (`hero.tsx` + `proof-wall-hero.tsx`) — flat `bg-brand-black` field, no gradient (the gradient is a secondary brand asset, not used here). A large mark animates in on load (fade/scale), then a single coordinated scroll-linked transform shrinks it, moves it toward the top-left, cycles it through color variants (gray/white/red/gradient, via the `Logo` component's `variant` prop — see Logo Behavior below), and fades it out, handing off to the header's wordmark (see #1). Cards live in their own grid column on each side of a fixed-width center column (`lg:grid-cols-[1fr_minmax(0,42rem)_1fr]`) so they can never overlap the headline text at any viewport — 1 left, 3 right, plus a 5th "top-center" card that floats above the grid in the open space between the nav and the headline (not part of either side column). Rather than a fixed on-load entrance, all five cards fly into their tilted resting positions together off the same `scrollYProgress` driving the logo shrink (`CARD_ENTRANCE_RANGE`, 0–0.35) — side cards slide in from the center, the top-center card slides up from behind the headline — so they arrive as one coordinated group timed with the logo's shrink, and are never in the large initial logo's way. Five tilted, sharp-cornered "paper" testimonial cards (off-white, tape accent, Caveat quote text, cursor-parallax) surround a central two-tone statement ("Parenting is **hard** for everyone. We shouldn't do it alone." — "hard" in `text-primary`), a regular-weight subhead, and a closing supporting line ("Father. Husband. Army veteran. Lived experience on every side of the system."), sourced live from the `testimonials` Keystatic collection (falls back to hardcoded copy if the collection is empty). CTA ("Tell Me What's Happening") is styled bymonolog.com-style — sharp corners, a small arrow icon, solid `bg-primary`/off-white text by default, inverting to off-white/`brand-black` on hover — and links straight out to the Cal.com `intro-call` URL, not an internal page.
   - **Logo Behavior:** the real BWPS SVG mark files live in `assets/logo/` — four icon color variants (`BWPSLogoGray/White/Red/Gradient.svg`) plus two subtitle wordmark files (`BWPSSubtitleOffWhite.svg`, `BWPSSubtitleGrey.svg`; a `BWPSSubtitleRed.svg` also exists, unused). `assets/logo/logo.tsx`'s `Logo` component has two render modes: the bare icon (`lockup={false}`, `variant` picks the color) used only by the Proof Wall hero's big scroll-cycling mark, and the default persistent two-part lockup (red icon + subtitle wordmark side by side) used everywhere else, including the header. The lockup's subtitle color is `onDark`-aware — off-white while still blended with a dark field, grey once on the light sticky header pill or any other page's header — this contrast rule is an interpretation of `homepage-redesign-v5.md`'s spec, not spelled out verbatim there. `proof-wall-hero.tsx` cycles the bare icon through all four variants via `useMotionValueEvent` on scroll progress as the hero logo shrinks/moves toward the header slot.
3. **Start Here** (`start-here.tsx`, async server component) — two rows, not a flat tile grid:
   - **Row 1:** both tiles are taller than a flat grid would default to. Left third is a Tech Safety Tool tile (`/tech-safety`) — an autoplay/muted/loop `<video>` (`public/videos/tech-safety-tool.mp4`) instead of a photo, with the title/subtitle overlaid bottom-left on a translucent `bg-primary/70` scrim for legibility over the moving footage (real photo was replaced by this video; no placeholder here anymore). Right two-thirds is a Common Pain Points box with a sticky, non-scrolling `bg-primary` header bar ("Common Pain Points" / "Find what's going on at your house.") and every pain-point topic from `getAllPainPoints()` in a plain `overflow-y-auto` list beneath it — scrolling with the cursor over the box moves the list via native browser scroll chaining (no custom JS), and releases back to page scroll automatically at the list's top/bottom edges; the same chaining applies to touch drags on mobile. No longer an auto-scrolling marquee — every pain-point link is still in the initial HTML (SEO requirement: no lazy-loading/fetch-on-interaction), it just doesn't auto-loop anymore.
...
4. **Quick Credential** (`quick-credential.tsx`) — real photo of Bobby (`assets/bobby-quick-credential.jpg`, static `next/image` import) + 5 bulleted credential lines (CPRS/RI Board ID, current role, military background, training, work history), "Learn more" → `/about-bobby`.
...
- **`/services` (For Parents):** Hero — a header ("Most families who find their way to me are lost.") and short subtitle ("Sometimes you can see a path...") lead into the big visual headline ("We Build [photo] Your Path", `components/marketing/services/wedged-hero-headline.tsx`), followed by a paragraph continuing the same material. The headline is a scroll-converge wedged-photo effect: both text halves and the photo are driven off one shared progress value (0–400px of raw page scroll from load, via `useScroll()`/`useTransform`, not an element-relative offset — an element-relative offset previously left the headline permanently stuck in its converged end state since this hero paints above the fold) — the two text halves translate inward and the photo scales up together, continuously, reaching close to a third of viewport width at full convergence (bigger than the bymonolog.com reference it's modeled on). The `[photo]` slot is `components/marketing/services/rotating-path-photo.tsx`, which crossfades through a fixed set of nature/trail photos (`assets/paths/*.jpg`, static imports) on a 3-second timer independent of scroll or viewport position — not a photo of Bobby. → "Start Walking Your Path" (the Stabilize/Connect/Structure/Adapt framework — this content lives here only, not duplicated on `/about-bobby`) → "What I Offer" (1:1 Sessions and Weekly Group are `available`; Cohorts is `comingSoon` with a "Join the waitlist" CTA that opens the Newsletter dialog; Live Q&As is `comingSoon` with its own interactive "Show Interest" email-capture widget — `components/marketing/services/interest-signup-widget.tsx`, `OfferCardData.cta.kind === "interest-signup"` — instead of the newsletter dialog: click slides open an email input, a valid email reveals a Submit button, submitting POSTs to `/api/interest-signups` (inserts into the `interest_signups` table, `source: "live_qa"`) and folds back into the button with a persistent green checkmark, tracked client-side via `localStorage` since there's no account system to look it up server-side) → closing CTA. A second "Talk to Bobby" CTA sits inside the Weekly Group card, in addition to the page's closing CTA — intentional, not a duplicate. The 1:1 Sessions and Weekly Group `OfferCard`s carry stable anchor IDs (`#one-on-one`, `#weekly-group` — `OfferCardData.anchorId`, rendered onto the `Card` with `scroll-mt-24`) so the homepage Start Here Row 2 panels ("Parents." / "Groups.") can deep-link straight to them.
...
- **Four Start Here / Row 2 photos still not supplied:** a parent using the Tech Safety Tool, a group session, an organization/nonprofit office, and an individual "parent" photo — `start-here.tsx` uses `PlaceholderPhoto` for all of these per `homepage-redesign-v3.md`'s own "Still needed" list. (Distinct from the Bobby headshots now wired into Quick Credential/About — these Row 2 photos are meant to depict parents/scenarios, not Bobby.)
