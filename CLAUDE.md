...
| `/legal` | Legal & Disclaimers — business registration, CPRS credential/ID, insurance, scope-of-practice, confidentiality/mandated-reporter, and not-a-crisis-service language | Live, static content (not CMS-managed), in `app/sitemap.ts`. Not in header/footer sitemap nav column — linked only as a small "Legal" link next to the footer's copyright line |
...
**CMS-managed:**
- `testimonials` — collection, `content/testimonials/*.yaml` (`quote`, `attribution`). `attribution` is a short, non-identifying descriptor (e.g. "Mom of 12 yr old boy, RI") rather than a generic "Parent peer-support client" label — chosen for feel of authenticity without disclosing which real client said what; not verified 1:1 against each client's actual details
...
- `footer` — singleton, `content/footer/data.yaml` (`tagline`, `contactEmail`, `copyrightText` — now also carries the CPRS/registered/insured disclaimer, not just the peer-support-not-therapy line). The sitemap/Connect link columns themselves are hardcoded in `footer.tsx` (a deliberately longer list than the header nav — see Homepage Structure) — not CMS-driven, since header/footer nav is a fixed design decision rather than free-form editable content. A hardcoded "Legal" link sits next to the copyright line, pointing to `/legal`.
- `about` — singleton, `content/about/data.yaml`. Fields: `heroSubhead`, `credentialBadge`, `photo` (real Keystatic image field — shows a placeholder box on the page until Bobby uploads one), `shortAboutParagraphs`, `cprsId`, `currentRole`, `training`, `prior`, `also`. The H1 ("About Bobby") is fixed copy, not a field. Bobby's full personal account is written directly into `shortAboutParagraphs`; there is no separate `personalDisclosure` field. The modalities statement only appears on `/services` ("My Approach"), not here. The "formal part" section's closing "Business" block (LLC registration + insurance line) is hardcoded copy in `app/about-bobby/page.tsx`, not a field — same pattern as the H1, since it's permanent legal fact rather than editable content.
...
- **`interest_signups`** (`email`, `source`, `created_at`, unique on `email`+`source`) — backs the Live Q&A "Show Interest" widget on `/services` (see Services Pages). Schema is in `lib/db/schema.ts`, migration is `drizzle/0002_minor_nextwave.sql`, and it's live — pushed to the real Neon database via `npm run db:push`, verified end-to-end against `/api/interest-signups`. The DB row is the sole source of truth (nothing reads back from the Sheet below). On each genuinely new signup (not a duplicate resubmit), `/api/interest-signups` also best-effort mirrors the row to a Google Sheet via `appendSheetRow` (`lib/google/sheets.ts`, reusing the `GOOGLE_SERVICE_ACCOUNT_*` credentials with the read-write `spreadsheets` scope) so Bobby can monitor new signups without querying Postgres — e.g. via a Claude scheduled task watching the Sheet. Gated on `INTEREST_SIGNUPS_SHEET_ID` (unset in dev by default); a Sheets failure is logged and swallowed, never blocks the signup.
