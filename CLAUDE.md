...
- `app/api/webhooks/drive-content-sync/route.ts` — the sync webhook. Auth: `X-Webhook-Secret` header, timing-safe compare against `WEBHOOK_SECRET` (same env var as `/api/refresh`). Body: `{fileId, fileName, folderKey}`. Two branches: `folderKey === "researchBibles"` + filename matching `RB_*.md` (bibles), and `folderKey === "parentFacingContent"` + filename matching `PainPoint_*.md` or `Module_*.md` (pain points / awareness modules) — anything else 400s. Bible flow: fetch from Drive → title extraction (typed `BibleParseError` on failure) → changelog-entry extraction (in-body Refinement Log block if present; otherwise falls back to the `_DDMMYY` date suffix on `fileName`; otherwise today's date with a genuine "initial sync" summary) → Pandoc-artifact strip → body extraction → sha256 dedup guard against the live GitHub file (no-op 200 if unchanged) → version computed server-side from changelog length → branch `content-sync/bible-<slug>-<date>` → commit → open PR → follow-up commit filling the real PR URL into the changelog entry. Pain-point/module flow: fetch from Drive → `status !== "ready"` skips silently (200, no-op, not an error) → parse + hard-validate (typed `ContentParseError` on failure) → serialize to the exact hand-authored YAML shape → dedup guard (pain points: canonicalized to ignore the site-owned `featured` line; modules: direct string compare) → branch `content-sync/pain-point-<slug>-<date>` or `content-sync/module-<slug>-<date>` → single commit → open PR listing changed top-level fields. Has an in-memory token-bucket rate limiter (module-level state, resets on cold start — acceptable for a low-traffic internal webhook, not a true distributed limiter).
- `lib/pain-points/parse.ts` — `getSourceStatus`, `parsePainPointSource`, `parseAwarenessModuleSource`, `ContentParseError`. Hand-rolled parser for the `PainPoint_*.md`/`Module_*.md` source-file contract (frontmatter + `## Heading` / `### Subheading` body sections) — validates `crisis` (explicit boolean), `icon` (one of 12 approved values), placeholder markers (`NEEDS SLUG`/`TODO`/`[GAP:`/`[ACCURACY FLAG:`), and `deepDiveHref`/`related[].href` (must start with `/docs/`, `/help/`, or be the bare `/docs`/`/help` index). Unit-tested (`parse.test.ts`, fixtures reverse-engineered from the real `meltdowns.yaml`/`modern.yaml`): `node --experimental-strip-types --test lib/pain-points/parse.test.ts`.
- `lib/pain-points/yaml.ts` — `serializePainPointYaml`/`serializeAwarenessModuleYaml`, reproducing the exact folded-block-scalar YAML style the 10 hand-authored files already use (not JSON-quoted flow scalars like the bible frontmatter serializer) — verified to match `meltdowns.yaml`'s structure line-for-line. Plus `extractFeaturedFlag`/`canonicalizeForDedup` (the pain-point `featured` site-owned-field handling) and `diffTopLevelFields` (produces the PR body's "changed fields" list via line-grouping, not a general YAML diff).
...
- `drive_content_sync_setup.md` (repo root) — Apps Script reference covering both the `researchBibles` and `parentFacingContent` Drive folders (one trigger, one shared last-run cutoff, per-folder filename pattern). Separate from `drive_sync_setup/`/`notify_change.gs`, which still serve the old (dormant) pipeline.
...
**Pain-point metadata rules:**
- `featured` (pain points only — `awarenessModules` has no equivalent) is site-owned, same pattern as the bible's `tags`. Defaults to `false` on first sync, never touched on resync — Bobby toggles it manually in Keystatic. Every other field on both collections is fully source-owned and gets overwritten on every sync.
...
**Still outstanding (pain-point/module half):**
- Nothing has been dropped in the `Parent Facing Content` Drive folder yet by the updated `parent-content-builder` skill — the parser and serializer are only verified against unit-test fixtures (reverse-engineered from the real `meltdowns.yaml`/`modern.yaml`) and one hand-run fixture, not a real skill-produced file.
- `serializePainPointYaml`/`serializeAwarenessModuleYaml` output hasn't been round-tripped through `/keystatic`'s own editor (same GitHub App env var blocker as the bible half).
- `drive_content_sync_setup.md`'s updated two-folder Apps Script isn't installed yet — manual step for Bobby in script.google.com: replace the existing script with the updated version, fill in both `FOLDERS[].driveFolderId` values (`researchBibles` = `1DYDwFPEyWFmsHR-XKvviajypNlDQedT2`, `parentFacingContent` = `111KCplYo8z-HccO-tnRRGr0apxtxQUeq`) and `CONFIG.webhookSecret`, then run `setup()` once.
